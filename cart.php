<?php 
  ...
  if (isset($_GET['action'])) {
    if ($_GET['action'] == "hapus" && isset($_GET['id'])) {
      ...
      $subtotal = $data_produk['harga'] * $_SESSION['keranjang_belanja'][$id_produk];
          ...
          <br><br><br><br>
            ...
            foreach ($_SESSION['keranjang_belanja'] as $id_produk => $jumlah_qty){
              ...
              while ($data = $query->fetch_assoc()) {
                $subtotal = $data['harga'] * $jumlah_qty;
           ...
           ?>         
			...
			<h3>Grand Total : Rp.<?=number_format($_SESSION['grand_total'])?> (Belum termasuk jasa pengiriman)</h3>
      <form action="checkout.php" method="post">
        <input type="hidden" name="grand_total" value="<?= $_SESSION['grand_total'] ?>">
        <label for="kurir">Pilih Jasa Pengiriman</label>
        <select name="kurir_product" class="form-control">
          <?php 
            $query_kurir = $koneksi->query("SELECT * FROM tb_jasa_pengiriman ORDER BY nama_jasa ASC");
            while($data_kurir = $query_kurir->fetch_assoc()){
              ?>
                <option value="<?= $data_kurir['id_jasa']?>">
                <?= $data_kurir['nama_jasa'] ."(Rp.". $data_kurir['tarif'].")"?>
                </option>
              <?php 
            }
          ?>
