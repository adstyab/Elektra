<?php 
  if(isset($_GET['search']) && !isset($_GET['categroy'])){
    $keyword = $_GET['search'];
    if(empty($keyword)){
    ...
    ?>
      ...
      <div class="col-lg-9">
          <br>
          <div class="jumbotron">
            <h1>Search Nama '<?= $keyword ?>'</h1>
          </div>

          <div class="row">
                <?php 
              $query_produk = $koneksi->query("SELECT * FROM tb_produk WHERE stok > 0 AND nama_produk LIKE '%$keyword%' ORDER BY tgl_produk DESC");
